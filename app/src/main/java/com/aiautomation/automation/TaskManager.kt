    ...
    suspend fun executeTask(task: Task): TaskStatus {
        try {
            ...
            while (stepCount < maxSteps && !isStopped) {
                ...
                // 检查是否陷入死循环（4次未变化直接终止）
                if (screenNoChangeCount >= 4) {
                    Log.e(TAG, "屏幕连续${screenNoChangeCount}次未变化，可能陷入死循环，终止任务")
                    AppLog.e(TAG, "🔁 死循环检测：屏幕${screenNoChangeCount}次未变化")
                    progressCallback?.onStatusUpdate("错误: 屏幕连续未变化，可能陷入死循环")
                    playErrorSound()
                ...
                val basePrompt = when {
                    screenNoChangeCount >= 2 -> {
                        "⚠️ 警告：屏幕已经${screenNoChangeCount}次没有变化！你可能陷入重复操作。必须立即换思路：" +
                        "\n1. 如果之前在点击 → 尝试滚动/back键/home键" +
                        "\n2. 如果之前在滚动 → 尝试点击其他区域/换方向/back键" +
                        "\n3. 如果卡住 → 按home键重新开始或使用替代方案" +
                        "\n4. 考虑通过浏览器/搜索引擎等通用方法解决" +
                        "\n分析当前屏幕，决定完全不同的操作。绝对不要重复！"
                    }
                    consecutiveFailures > 0 -> {
                        "上次操作失败 ($consecutiveFailures 次)，请换一种完全不同的方法：" +
                        "\n- 点击不同的元素/位置" +
                        "\n- 使用滚动/滑动" +
                        "\n- 按back/home键" +
                        "\n- 使用搜索功能" +
...
- 如果点击失败 → 尝试滚动/返回键/点击其他位置
- 如果滚动无效 → 尝试点击/换方向/返回键
- 如果输入失败 → 点击其他输入框/确认按钮/重新点击
